import { compressDeck, decompressDeck, matrixMultiplication } from "./utils"
...
///     c11.y, ..., c1n.y,
/// ]
export function shuffleEncryptPlaintext(babyjub: BabyJub, numCards: number, A: bigint[], X: bigint[], R: bigint[], pk: EC): bigint[] {
    let B: bigint[] = [];
    for (let i = 0; i < 4; i++) {
        let tmp: bigint[] = matrixMultiplication(A, X.slice(i * numCards, (i + 1) * numCards), numCards, numCards);
...
export function shuffleEncryptV2Plaintext(
    ...
    UY1_delta: bigint[],
    s_u: bigint[],
): { X0: bigint[], X1: bigint[], delta0: bigint[], delta1: bigint[], selector: bigint[] } {
    let U = decompressDeck(UX0, UX1, UY0_delta, UY1_delta, s_u);
