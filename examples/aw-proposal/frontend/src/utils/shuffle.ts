...
const privateKey =
  '0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80'; // the private key of the account that deploys the game contract
...
const stateMachineContract = new ethers.Contract(
  contracts.Shuffle.address,
  contracts.Shuffle.abi,
  provider
);
...
export async function registerPlayers(numPlayers: bigint) {
  ...
  for (let i = 0; i < numPlayers; i++) {
    await stateMachineContract
      .connect(signer)
      .register(playerAddrs[i], [pkArray[i][0], pkArray[i][1]], gameId);
  }
}
