...
export function keyGen(
  ...
  const sk = sampleFieldElements(babyjub, numBits, BigInt(1))[0];
  return {
    g: babyjub.Base8,
    sk: sk,
...
export function samplePermutation(n: number): bigint[] {
  let array = Array.from({ length: n }).map((_, i) => i);
...
/// Converts the type of pks to be BigNumberish string.
export function convertPk(babyjub: BabyJub, pks: EC[]): string[][] {
  let arr = [];
  for (let i = 0; i < pks.length; i++) {
    let pk: string[] = [];
...
export function ecDecompress(
  ...
  for (let i = 0; i < selectorArr.length; i++) {
    ...
    ecArr.push(flag * deltaArr[i] + (BigInt(1) - flag) * (q - deltaArr[i]));
  }
  return ecArr;
}
...
/// Converts an array of bignumberish to an array of bigint.
export function bignumberish2Bigint(arr: BigNumberish[]): bigint[] {
  let output: bigint[] = [];
  for (let i = 0; i < arr.length; i++) {
    output.push(BigNumber.from(arr[i]).toBigInt());
  }
  return output;
}
...
export function prepareDecryptData(
  ...
  Y.push(x0.toBigInt());
  Y.push(flag0 * delta0 + (BigInt(1) - flag0) * (q - delta0));
  Y.push(x1.toBigInt());
  Y.push(flag1 * delta1 + (BigInt(1) - flag1) * (q - delta1));
  return Y;
}
