...
describe("ZKShuffle Unit Test", function () {
  ...
  it("Create Shuffle Game", async () => {
    async function createShuffleGame(numPlayer: number, owner: Signer) {
  ...
  it("Move to Register State", async () => {
    async function moveToRegister(gameId: number, owner: Signer) {
  ...
  it("Player Register", async () => {
    ...
    for (let i = 0; i < numPlayer; i++) {
      await playerRegister(
        gameId,
        await players[i].signer.getAddress(),
        players[i].pk[0],
        players[i].pk[1],
        players[i].signer,
      );
  ...
  it("Move to Shuffle", async () => {
    async function moveToShuffle(gameId: number, owner: Signer) {
      const next = game.interface.encodeFunctionData("dummy");
