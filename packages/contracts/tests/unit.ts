...
describe("zkShuffle Unit Test", function () {
  ...
  it("Create Shuffle Game", async () => {
    async function createShuffleGame(
      ...
      const calldata = SM.interface.encodeFunctionData("createShuffleGame", [
        numPlayer,
      ]);
  ...
  it("Move to Register State", async () => {
    async function moveToRegister(gameId: number, owner: SignerWithAddress) {
      const next = game.interface.encodeFunctionData("dummy");
      const calldata = SM.interface.encodeFunctionData("register", [
        gameId,
        next,
      ]);
      await tx_to_contract(owner, game.address, calldata);
    ...
    expect((await SM.gameState(gameId)).toNumber()).equal(
      BaseState.Registration
    );
  });
  ...
  it("Move to Shuffle", async () => {
    async function moveToShuffle(gameId: number, owner: SignerWithAddress) {
      const next = game.interface.encodeFunctionData("dummy");
      const calldata = SM.interface.encodeFunctionData("shuffle", [
        gameId,
        next,
      ]);
      await tx_to_contract(owner, game.address, calldata);
  ...
  it("Player Shuffle", async () => {
    async function playerShuffle(gameId: number, player: zkShuffle) {
...
describe("zkShuffle State Less Unit Test", function () {
  ...
  it("Player Register StateLess", async () => {
    ...
    async function playerRegister(pid: number) {
      ...
      return await ShuffleManager__factory.connect(
        SM.address,
        player.owner
      ).playerRegister(
        gameId,
    ...
    await expect(playerRegister(0))
      .to.emit(SM, "Register")
      .withArgs(gameId, 0, players[0].owner.address);
    await expect(playerRegister(1))
      .to.emit(SM, "Register")
      .withArgs(gameId, 1, players[1].owner.address);
