...
describe('zkShuffle State Less Unit Test', function () {
    ...
    it('Player Shuffle', async () => {
        ...
        //  prerequisite 2 : Init Game State : Shuffle
        await SM.set_gameState(gameId, BaseState.Shuffle)
        expect((await SM.gameState(gameId)).toNumber()).equal(BaseState.Shuffle)

        //  prerequisite 3 : Init Deck
        await SM.initDeck(gameId)

        //  prerequisite 4 : Set Player's Turn
        await SM.set_playerTurn(gameId, players[0].owner.address)
        ...
        async function playerShuffle(
            ...
            return await ShuffleManager__factory.connect(SM.address, player.owner).playerShuffle(
                gameId,
                solidityProof,
                compressDeck
            )
        ...
        await expect(playerShuffle(0)).to.emit(SM, "PlayerTurn").withArgs(gameId, 1, BaseState.Shuffle)
    });
